...
import {
  hasTestSucceeded,
  isCriticalError
} from '@datadog/datadog-ci/dist/commands/synthetics/utils'
...
export const renderResults = (
  ...
  for (const test of tests) {
    ...
    const hasTimeout = testResults.some(
      pollResult => pollResult.result.error === ERRORS.TIMEOUT
    )
    if (hasTimeout) {
      summary.timedOut++
    }
    ...
    if (!config.failOnCriticalErrors) {
      const hasCriticalErrors = testResults.some(pollResult =>
        isCriticalError(pollResult.result)
      )
...
export const sortTestsByOutcome = (
  ...
  const isNonBlockingTest1 =
    t1.options.ci?.executionRule === ExecutionRule.NON_BLOCKING
  const isNonBlockingTest2 =
    t2.options.ci?.executionRule === ExecutionRule.NON_BLOCKING
