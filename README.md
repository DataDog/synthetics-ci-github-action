...
| `fail_on_missing_tests`   | boolean | _optional_  | Fail the CI job if at least one test is missing in a run (for example, if it has been removed or deleted). **Default:** `false`.                                                                         |
