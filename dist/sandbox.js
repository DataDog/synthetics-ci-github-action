	...
	for (let i = 0; i < vm.options.sourceExtensions.length; i++) {
		...
		EXTENSIONS['.' + ext] = (module, filename, dirname) => {
			if (vm.options.require.context !== 'sandbox') {
				...
				// run the script
				closure(module.exports, __nccwpck_require__(64), module, filename, dirname);
			}
		};
	}
	...
	const _requireBuiltin = (moduleName) => {
		...
		if (moduleName === 'events' || moduleName === 'internal/errors') {
			...
			try {
				...
				script.runInContext(global)(module.exports, __nccwpck_require__(64), module, host.process, host.process.binding);
			} catch (e) {
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(794);
